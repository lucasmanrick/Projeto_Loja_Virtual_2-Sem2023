...
const carrinhoAdd = (req, res) => {
   ...
   if(existClient) {
      ...
      if(receiveProduct) { 
        ...
        if(checkCart) {
          const newCart = new Venda (crud.myData[0].id,crud.myData[0].data,crud.myData[0].valorTotal) 
          ...
          const newCart = new Venda (1,new Date (),receiveProduct.preco,receiveProduct)
          newCart.adicionarAoCarrinho(receiveProduct)
          crud.create(newCart,'./data/carrinho.JSON')
          res.render("carrinho",{dataClient:existClient,dataCart:newCart})
        }
